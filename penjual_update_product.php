...
if(isset($_POST['update_product'])){
   ...
   $update_product = $conn->prepare("UPDATE `produkpenjual` SET name = ?, category = ?, details = ?, price = ? WHERE id = ?");
   ...
   if(!empty($image)){
      if($image_size > 2000000){
         ...
         $update_image = $conn->prepare("UPDATE `podukpenjual` SET image = ? WHERE id = ?");
...
<section class="update-product">
   ...
   <?php
      ...
      $select_products = $conn->prepare("SELECT * FROM `produkpenjual` WHERE id = ?");
